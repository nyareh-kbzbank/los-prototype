...
const normalizeScoreCard = (card) => {
  const normalizedFields = (card.fields ?? []).map((field) => {
    ...
    return { ...field, description, rules };
  });
  if (!normalizedFields.length && card.rules) {
    const legacyRules = card.rules;
    const byField = {};
...
const useScoreCardStore = create()(
  persist(
    (set) => ({
      ...
      upsertScoreCard: (next, opts) => set((prev) => {
        const normalized = normalizeScoreCard(next);
        return {
          scoreCards: { ...prev.scoreCards, [normalized.scoreCardId]: normalized },
          selectedScoreCardId: opts?.select === false ? prev.selectedScoreCardId : normalized.scoreCardId
        };
      }),
      removeScoreCard: (scoreCardId) => set((prev) => {
        const { [scoreCardId]: _removed, ...rest } = prev.scoreCards;
        const remainingIds = Object.keys(rest);
        const nextSelected = prev.selectedScoreCardId === scoreCardId ? remainingIds[0] ?? defaultScoreCard.scoreCardId : prev.selectedScoreCardId;
        const nextCards = remainingIds.length === 0 ? { [defaultScoreCard.scoreCardId]: normalizeScoreCard(defaultScoreCard) } : rest;
        return {
          scoreCards: nextCards,
          selectedScoreCardId: nextSelected
        };
      }),
      selectScoreCard: (scoreCardId) => set(
        (prev) => prev.scoreCards[scoreCardId] ? { selectedScoreCardId: scoreCardId } : prev
      ),
      resetStore: () => set({
        scoreCards: {
          [defaultScoreCard.scoreCardId]: normalizeScoreCard(defaultScoreCard)
    ...
    }),
    {
      ...
      migrate: (persistedState) => {
        ...
        for (const [id, card] of Object.entries(state?.scoreCards ?? {})) {
          normalizedCards[id] = normalizeScoreCard(card);
        }
        const selected = normalizedCards[state?.selectedScoreCardId ?? ""] ? state?.selectedScoreCardId : Object.keys(normalizedCards)[0] ?? defaultScoreCard.scoreCardId;
        return {
          scoreCards: Object.keys(normalizedCards).length > 0 ? normalizedCards : { [defaultScoreCard.scoreCardId]: normalizeScoreCard(defaultScoreCard) },
...
export {
  operatorOptions as o,
  useScoreCardStore as u
};
