...
export const Route = createFileRoute("/loan/checker-inbox/$applicationId")({
	component: CheckerInboxDetailPage,
});

function CheckerInboxDetailPage() {
	...
	const handleApprove = () => {
		...
		navigate({ to: "/loan/checker-inbox" });
	};
	...
	return (
		<div className="p-6 font-sans max-w-3xl mx-auto space-y-4">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-bold">Checker review detail</h1>
			...
			<section className="border rounded p-4 text-sm space-y-3">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					...
					<Field
						label="Product"
						value={application.productName ?? application.productCode}
					/>
					<Field
			...
			<section className="border rounded p-4 text-sm space-y-3">
				...
				{canTakeDecision ? (
					...
					<div className="text-sm text-gray-600">
						Checker actions are available only for applications waiting in checker inbox.
					</div>
				)}
			</section>
		</div>
