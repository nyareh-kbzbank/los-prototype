...
export const Route = createFileRoute("/loan/checker-inbox/")({
	component: CheckerInboxPage,
});

function CheckerInboxPage() {
	const applications = useLoanApplicationStore((s) => s.applications);
	const rows = useMemo(
		() =>
			getLoanApplicationList(applications).filter(
				(application) => application.status === "CHECKER_PENDING",
			),
		[applications],
	);
	...
	return (
		<div className="p-6 font-sans max-w-5xl mx-auto space-y-4">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-bold">Checker Inbox</h1>
					<p className="text-sm text-gray-700">
						Applications submitted by maker and waiting for checker decision.
					</p>
				</div>
				<Link
			...
			{rows.length === 0 ? (
				<div className="border rounded p-4 bg-gray-50 text-sm text-gray-700">
					No applications are waiting in checker inbox.
				</div>
			) : (
