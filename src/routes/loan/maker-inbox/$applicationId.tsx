...
export const Route = createFileRoute("/loan/maker-inbox/$applicationId")({
	component: MakerInboxDetailPage,
});

function MakerInboxDetailPage() {
	...
	const handleSubmitToChecker = () => {
		...
		navigate({
			to: "/loan/checker-inbox/$applicationId",
		...
		return (
			<div className="p-6 font-sans max-w-3xl mx-auto">
				<div className="border rounded p-4 bg-red-50 text-sm text-gray-800">
					...
					<div className="mt-2">
						<Link to="/loan/maker-inbox" className="text-blue-600 hover:underline">
							Back to maker inbox
						</Link>
					</div>
				</div>
	...
	return (
		<div className="p-6 font-sans max-w-3xl mx-auto space-y-4">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-bold">Maker review detail</h1>
			...
			<section className="border rounded p-4 text-sm space-y-3">
				...
				{canTakeDecision ? (
					...
					<div className="text-sm text-gray-600">
						Maker actions are available only for applications waiting in maker inbox.
					</div>
				)}
			</section>
		</div>
