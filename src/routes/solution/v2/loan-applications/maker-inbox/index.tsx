...
export const Route = createFileRoute("/solution/v2/loan-applications/maker-inbox/")({
	component: V2MakerInboxPage,
});

function V2MakerInboxPage() {
	...
	const rows = useMemo(() => {
		...
		return getLoanApplicationList(applications).filter(
			(application) =>
				v2SetupIds.has(application.setupId) && application.status === "SUBMITTED",
		);
	}, [applications, setups]);
	...
	return (
		<div className="p-6 font-sans max-w-5xl mx-auto space-y-4">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-bold">V2 Maker Inbox</h1>
					<p className="text-sm text-gray-700">
						V2 applications waiting for maker decision.
					</p>
				</div>
			...
			{rows.length === 0 ? (
				<div className="border rounded p-4 bg-gray-50 text-sm text-gray-700">
					No V2 applications are waiting in maker inbox.
				</div>
			) : (
